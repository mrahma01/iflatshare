...
class AbsentProfileAdmin(admin.ModelAdmin):
    def queryset(self, request):
        qs = super(AbsentProfileAdmin, self).queryset(request)
        if request.user.is_superuser:
            return qs
        return qs.filter(user__address=request.user.profile.address)

    def formfield_for_foreignkey(self, db_field, request=None, **kwargs):
        if not request.user.is_superuser:
            kwargs['queryset'] = request.user.profile.get_housemate_profiles()
            return super(AbsentProfileAdmin, self).formfield_for_foreignkey(db_field,\
                                                            request, **kwargs)
        return super(AbsentProfileAdmin, self).formfield_for_foreignkey(db_field,\
                                                            request, **kwargs)
class AbsentInline(admin.TabularInline):
    model = AbsentProfile
    extra = 0

class ProfileAdmin(admin.ModelAdmin):
    inlines = [AbsentInline]
...
admin.site.register(AbsentProfile, AbsentProfileAdmin)
